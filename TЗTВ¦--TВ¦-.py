...
@app.route('/delete_film/<int:film_id>/<int:room_id>/<int:cinema_id>/<user_id>', methods=['GET'])
def delete_film(film_id, room_id, cinema_id, user_id):
    if 'username' not in session:
        return redirect('/login')
    nm = FilmModel(db.get_connection())
    nm.delete(film_id)
    return redirect("/films/{}/{}/{}".format(room_id, cinema_id, user_id))
